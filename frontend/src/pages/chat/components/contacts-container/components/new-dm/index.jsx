    ...
    const searchContacts = async (searchTerm) => {
        try {
            if (searchTerm.length > 0) {
                const resposne = await apiClient.post(SEARCH_CONTACTS_ROUTES, { searchTerm }, { withCredentials: true })

                if (resposne.status === 200 && resposne.data.contacts) {
                    setSearchedContacts(resposne.data.contacts);
                }
            } else {
                setSearchedContacts([])
            }
                ...
                <DialogContent className="bg-[#181920] border-none text-white w-[400px] h-[400px] flex flex-col">
...

                    {
                        searchedContacts.length <= 0 && (
                            <div className="flex-1 md:bg-[#1c1d25] md:flex mt-3 flex-col justify-center items-center duration-1000 transition-all">
                                <Player
                                    autoplay
                                    loop
                                    src={animationData}
